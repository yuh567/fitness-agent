...
- 精力水平：${todayStatus.energyLevel ?? '未知'}/10
- 备注：${todayStatus.notes ?? '无'}

最近训练记录：
${recentLogs.take(5).map((l) => '- ${l.createdAt ?? '未知'}: ${l.actualWeight}kg × ${l.actualSets}组').join('\n')}

要求：
1. 如精力水平低（≤3），建议减量或改为恢复训练
...
'''
  ...
  Future<Map<String, dynamic>?> requestDeepAdjustment() async {
    final db = DBHelper();
    final logs = await db.query('workout_logs', orderBy: 'created_at DESC');
